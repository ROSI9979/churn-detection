                    ...
                    {customers.sort((a: any, b: any) => (b.churn_risk_score || 0) - (a.churn_risk_score || 0)).slice(0, 15).map((c: any, i: number) => (
                      <tr key={c.customer_id} className="border-b border-slate-700 hover:bg-slate-900 transition">
                        ...
                        <td className="px-4 py-3 text-center">
                          <span className={`px-3 py-1 rounded-full text-xs font-bold ${c.churn_risk_score >= 75 ? 'bg-red-900 text-red-200' : c.churn_risk_score >= 50 ? 'bg-orange-900 text-orange-200' : 'bg-green-900 text-green-200'}`}>
                            {c.churn_risk_score?.toFixed(0) || 0}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-center">
                          {c.churn_risk_score >= 75 ? '🔴 Critical' : c.churn_risk_score >= 50 ? '🟠 Warning' : '🟢 Healthy'}
                        </td>
                      </tr>
                    ))}
                  </tbody>
